...
final class KOZDONATE_Plugin {
	public const OPTION = 'kozdonate_settings';
	public const PAGE_SLUG = 'koz-donate-stats';
	public const CLEANUP_HOOK = 'kozdonate_cleanup';
	public const EXPORT_CSV_ACTION = 'kozdonate_export_csv';
	public const EXPORT_JSON_ACTION = 'kozdonate_export_json';
	public const RESET_ACTION = 'kozdonate_reset';
	public const ROTATE_SECRET_ACTION = 'kozdonate_rotate_secret';
	public const RESET_PHRASE = 'DELETE DONATION STATS';
	public const LAST_CONFIRMATION_OPTION = 'kozdonate_last_confirmation_at';
	public const SETTINGS_VERSION = 6;

	private static function legacy_prefix(): string {
		return implode( '', array( 'ua', 'free' ) );
	}

	private static function legacy_option_name(): string {
		return self::legacy_prefix() . '_donate_stats_settings';
	}

	private static function legacy_confirmation_option(): string {
		return self::legacy_prefix() . '_donate_stats_last_confirmation_at';
	}

	private static function legacy_cleanup_hook(): string {
		return self::legacy_prefix() . '_donate_stats_cleanup';
	}
	...
	private static function maybe_migrate_legacy_options(): void {
		...
		if ( is_array( $legacy ) && false === get_option( 'kozdonate_legacy_settings_migrated_138', false ) ) {
			$current_array = is_array( $current ) ? $current : array();
			// Historical administrator choices win; current-only fields are kept as fallback.
			$merged = wp_parse_args( $legacy, $current_array );
			update_option( self::OPTION, $merged, false );
			update_option( 'kozdonate_legacy_settings_migrated_138', 1, false );
		} elseif ( false === $current && is_array( $legacy ) ) {
			add_option( self::OPTION, $legacy, '', false );
		}

		if ( false === get_option( self::LAST_CONFIRMATION_OPTION, false ) ) {
			$legacy_confirmation = get_option( self::legacy_confirmation_option(), false );
			if ( false !== $legacy_confirmation ) {
				add_option( self::LAST_CONFIRMATION_OPTION, $legacy_confirmation, '', false );
			}
		}
	}
	...
	public static function init(): void {
		...
		add_filter( 'kozdonate_public_status', array( KOZDONATE_Tracker::class, 'public_status' ) );

		KOZDONATE_Tracker::init();
		KOZDONATE_Admin::init();
	}
	...
	public static function defaults(): array {
		return array(
			...
			'data_layer_event'    => 'kozdonate_donation_event',
	...
	public static function activate(): void {
		self::maybe_migrate_legacy_options();
		KOZDONATE_Storage::install_schema();
	...
	public static function maybe_upgrade(): void {
		if ( KOZDONATE_Storage::DB_VERSION !== (string) get_option( 'kozdonate_db_version', '' ) ) {
			KOZDONATE_Storage::install_schema();
		}
	...
	public static function sanitize_settings( $input ): array {
		...
		$current = get_option( self::OPTION, array() );
		$current = is_array( $current ) ? $current : array();
		$confirmation_secret = strtolower( trim( (string) ( $input['confirmation_secret'] ?? $current['confirmation_secret'] ?? '' ) ) );
	...
	/**
	 * Return active KOZ Static Translate language slugs.
	 *
	 * @return array<int,string>
	 */
	public static function static_translate_languages(): array {
		$legacy_status_function = implode( '_', array( 'uafree', 'static', 'translate', 'get', 'status' ) );
		if ( ! is_callable( $legacy_status_function ) ) {
			return array();
		}

		try {
			$status = call_user_func( $legacy_status_function );
	...
	public static function confirmation_callback_url(): string {
		return rest_url( KOZDONATE_Tracker::REST_NAMESPACE . '/confirm' );
	}
	...
	public static function cleanup_old_data(): void {
		KOZDONATE_Storage::cleanup_old_data( (int) self::settings()['retention_days'] );
	}
}
