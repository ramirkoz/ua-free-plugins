...
final class KOZDONATE_Tracker {
	public const REST_NAMESPACE = 'kozdonate/v1';

	public static function init(): void {
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'enqueue' ), 50 );
		add_action( 'rest_api_init', array( __CLASS__, 'register_rest_routes' ) );
		add_action( 'kozdonate_record_server_event', array( __CLASS__, 'record_server_event' ), 10, 4 );
	}
	...
	public static function enqueue(): void {
		...
		wp_enqueue_script(
			'kozdonate-frontend',
			KOZDONATE_URL . 'assets/frontend.js',
			array(),
			KOZDONATE_VERSION,
			true
		);
		...
		if ( class_exists( '\\ramirkz\\kozcopyactions\\KOZCOAC_Plugin' ) ) {
			$copy_settings = \ramirkz\kozcopyactions\KOZCOAC_Plugin::settings();
		...
		$config = array(
			...
			'pluginVersion'      => KOZDONATE_VERSION,
		);

		$config = apply_filters( 'kozdonate_frontend_config', $config, $context, $settings );
	...
	public static function context_for_request( ?array $settings = null ) {
		$settings = $settings ?? KOZDONATE_Plugin::settings();
		...
		if ( ! $context && ! empty( $settings['include_static_translations'] ) ) {
			...
			foreach ( KOZDONATE_Plugin::translated_routes( (array) $settings['tracked_page_ids'] ) as $route ) {
		...
		$context = apply_filters( 'kozdonate_request_context', $context, $settings );
		...
		$should_track = apply_filters( 'kozdonate_should_track', true, $context, $settings );
	...
	public static function record_event( WP_REST_Request $request ): WP_REST_Response {
		...
		KOZDONATE_Storage::store_session( $language, $context_key, $session_hash );
		$accepted = KOZDONATE_Storage::increment_event( $language, $context_key, $event_type, $target_key );

		if ( $accepted ) {
			do_action(
				'kozdonate_event_recorded',
				array(
	...
	public static function confirm_donation( WP_REST_Request $request ): WP_REST_Response {
		...
		$signature = (string) $request->get_header( 'x-kozdonate-signature' );
		...
		$status = KOZDONATE_Storage::record_confirmation(
	...
	private static function within_rate_limit( string $session_hash, string $context_key ): bool {
		$session_key = 'kozdonate_rate_' . substr( $session_hash, 0, 32 );
		...
		$event_key = 'kozdonate_events_' . $server_bucket;
		$new_session_key = 'kozdonate_new_sessions_' . $server_bucket;
		$seen_key = 'kozdonate_seen_' . $server_bucket . '_' . substr( $session_hash, 0, 16 );
	...
	public static function public_status( $value = null ): array {
		$settings = KOZDONATE_Plugin::settings();
		$counts = KOZDONATE_Storage::table_counts();
		...
		return array(
			...
			'confirmation_ready'   => ! empty( KOZDONATE_Plugin::confirmation_status()['ready'] ),
			'last_confirmation_at' => (string) KOZDONATE_Plugin::confirmation_status()['last_confirmation_at'],
