...
final class KOZDONATE_Storage {
	public const DB_VERSION = '1.1.1';
	public const DAILY_SUFFIX = 'kozdonate_daily';
	public const SESSIONS_SUFFIX = 'kozdonate_sessions';
	public const CONFIRMATIONS_SUFFIX = 'kozdonate_confirmations';

	private static string $daily_table = '';
	private static string $sessions_table = '';
	private static string $confirmations_table = '';

	private static function legacy_suffix( string $kind ): string {
		return implode( '', array( 'ua', 'free' ) ) . '_donate_' . $kind;
	}
	...
	private static function migrate_legacy_tables(): void {
		...
		if ( false !== get_option( 'kozdonate_legacy_tables_migrated_138', false ) ) {
			return;
		}

		$legacy_daily = $wpdb->prefix . self::legacy_suffix( 'daily' );
		$legacy_sessions = $wpdb->prefix . self::legacy_suffix( 'sessions' );
		$legacy_confirmations = $wpdb->prefix . self::legacy_suffix( 'confirmations' );
		$ok = true;

		if ( self::table_exists( $legacy_daily ) ) {
			$ok = false !== $wpdb->query(
				$wpdb->prepare(
					'INSERT INTO %i (stat_date,language,context_key,event_type,target_key,event_count,created_at,updated_at)
					 SELECT stat_date,language,context_key,event_type,target_key,event_count,created_at,updated_at FROM %i
					 ON DUPLICATE KEY UPDATE event_count=event_count+VALUES(event_count), created_at=LEAST(created_at,VALUES(created_at)), updated_at=GREATEST(updated_at,VALUES(updated_at))',
					self::$daily_table,
					$legacy_daily
				)
			) && $ok;
		...
		if ( self::table_exists( $legacy_sessions ) ) {
			$ok = false !== $wpdb->query(
				$wpdb->prepare(
					'INSERT IGNORE INTO %i (stat_date,language,context_key,session_hash,created_at)
					 SELECT stat_date,language,context_key,session_hash,created_at FROM %i',
					self::$sessions_table,
					$legacy_sessions
				)
			) && $ok;
		...
		if ( self::table_exists( $legacy_confirmations ) ) {
			$ok = false !== $wpdb->query(
				$wpdb->prepare(
					'INSERT IGNORE INTO %i (reference_hash,provider,language,context_key,created_at)
					 SELECT reference_hash,provider,language,context_key,created_at FROM %i',
		...
		if ( $ok ) {
			update_option( 'kozdonate_legacy_tables_migrated_138', 1, false );
		}
	}
	...
	public static function install_schema(): void {
		...
		update_option( 'kozdonate_db_version', self::DB_VERSION, false );
	}
	...
	public static function record_confirmation(
		...
		update_option(
			KOZDONATE_Plugin::LAST_CONFIRMATION_OPTION,
			current_time( 'mysql', true ),
			false
		);
		return 'recorded';
	...
	public static function reset(): void {
		...
		delete_option( KOZDONATE_Plugin::LAST_CONFIRMATION_OPTION );
	}
	...
	public static function summary( int $days = 30 ): array {
		...
		return apply_filters( 'kozdonate_summary', $result, $days );
	}
