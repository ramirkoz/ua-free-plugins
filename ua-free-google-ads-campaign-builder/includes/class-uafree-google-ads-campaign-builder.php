<?php
namespace UAFree\GoogleAdsCampaignBuilder;
 ...
 */
final class Plugin {
	private const PAGE_SLUG       = 'ua-free-google-ads-campaign-builder';
	private const SETTINGS_OPTION = 'uafree_agb_settings';
	private const PACKAGE_OPTION  = 'uafree_agb_last_package';
	private const ACTION_SAVE     = 'uafree_agb_save';
	...
	public static function init(): void {
		...
		add_filter( 'plugin_action_links_' . plugin_basename( UAFREE_GACB_FILE ), array( __CLASS__, 'action_links' ) );
	}

	public static function admin_menu(): void {
		global $admin_page_hooks;
		if ( empty( $admin_page_hooks['uafree-suite'] ) ) {
			add_menu_page( 'UA FREE', 'UA FREE', 'manage_options', 'uafree-suite', array( __CLASS__, 'admin_page' ), 'dashicons-admin-plugins', 58 );
		}
		add_submenu_page(
			'uafree-suite',
	...
	public static function admin_assets( string $hook_suffix ): void {
		...
		wp_enqueue_style(
			'uafree-google-ads-campaign-builder-admin',
			plugins_url( 'assets/admin.css', UAFREE_GACB_FILE ),
			array(),
			UAFREE_GACB_VERSION
		);
	}
	...
	/**
	 * Privacy-safe, read-only state for other UA FREE components.
	 *
	...
	public static function get_status(): array {
		...
		return array(
			'version'                => defined( 'UAFREE_GACB_VERSION' ) ? UAFREE_GACB_VERSION : '',
	...
	private static function detected_site_language(): array {
		$translator = get_option( 'uafree_st_auto_settings', array() );
		...
		if ( isset( self::$languages[ $slug ] ) ) {
			return array( 'slug' => $slug, 'source' => 'UA FREE Static Translate' );
		}
	...
	private static function redirect_notice( string $notice, string $message = '' ): void {
		$args = array( 'page' => self::PAGE_SLUG, 'uafree_agb_notice' => sanitize_key( $notice ) );
		if ( '' !== $message ) {
	...
	/**
	 * Automatic campaign assets restored from the original fund plugin.
	 *
	 * @return array{callouts:string[],negative_keywords:string[]}
	 */
	private static function automatic_assets_for_language(
		string $language,
		array $settings,
		array $credentials
	): array {
		...
		if (
			! empty( $settings['allow_azure_copy_translation'] )
			&& ! empty( $credentials['configured'] )
		) {
			$flat = array_merge(
				$english['callouts'],
				$english['negative_keywords']
			);
			$translated = self::translate_strings(
				...
				$language,
				$credentials,
				(string) $settings['organisation_name']
			);

			if ( ! is_wp_error( $translated ) ) {
				$callout_count = count( $english['callouts'] );
		...
		/*
		 * A package must remain self-contained even when Azure is unavailable.
		 * English is the neutral built-in fallback for unsupported languages.
		 */
		return $english;
	}
	...
	private static function build_plan( array $settings, string $generation_id ) {
		...
		foreach ( self::campaign_languages( $settings ) as $language ) {
			...
			$automatic_assets = self::automatic_assets_for_language(
				$language,
				$settings,
				$credentials
			);
	...
	public static function generate_package(): void {
		...
		$manifest = array(
			'generator' => array( 'name' => 'UA FREE Google Ads Campaign Builder', 'version' => UAFREE_GACB_VERSION, 'generated_at_utc' => gmdate( 'c' ), 'package_id' => $generation_id ),
			'account_mode' => self::account_mode( $settings ),
	...
	private static function is_ukrainian(): bool {
		$locale = function_exists( 'determine_locale' ) ? determine_locale() : get_locale();
		return str_starts_with( strtolower( (string) $locale ), 'uk' );
	}
	...
	private static function manager_items( string $title, array $items, string $empty = '' ): void {
		?>
		<section class="uafree-agb-section">
			<h2><?php echo esc_html( $title ); ?></h2>
			<?php if ( empty( $items ) ) : ?>
				<p class="uafree-agb-empty"><?php echo esc_html( $empty ); ?></p>
			<?php else : ?>
				<div class="uafree-agb-list">
					<?php foreach ( $items as $item ) : ?>
						<article class="uafree-agb-item is-<?php echo esc_attr( (string) ( $item['level'] ?? 'success' ) ); ?>">
	...
	private static function render_overview_tab(
		...
		?>
		<div class="uafree-agb-hero is-<?php echo esc_attr( (string) $summary['overall'] ); ?>">
		...
		<section class="uafree-agb-generate">
		...
		<?php if ( is_array( $package ) && ! empty( $package['path'] ) && is_file( (string) $package['path'] ) ) : ?>
			<section class="uafree-agb-last-package">
				<h2><?php echo esc_html( self::ui( 'Останній пакет', 'Last package' ) ); ?></h2>
				<div class="uafree-agb-package-grid">
		...
		<details class="uafree-agb-technical">
	...
	private static function render_settings_tab( array $settings ): void {
		?>
		<div class="uafree-agb-settings-intro">
				...
				<table class="form-table" role="presentation">
					...
					<tr>
						...
						<td>
							...
							<fieldset class="uafree-agb-country-grid">
