...
final class KOZ404_Suite_Registry {
	private const FALLBACK_PAGE = 'koz404-suite';
	...
	public static function suite_page(): string {
		...
		add_menu_page(
			__( 'KOZ WordPress Suite', 'koz-404-guard' ),
			__( 'KOZ Suite', 'koz-404-guard' ),
	...
	public static function catalog(): array {
		return array(
			...
			'koz-404-guard' => array( 'name' => 'KOZ 404 Guard & URL Intelligence', 'legacy' => array( 'ua-free-seo-core' ), 'settings_page' => 'koz-404-guard' ),
	...
	public static function render(): void {
		...
		?>
		<div class="wrap koz404-suite-overview"><h1><?php echo esc_html__( 'KOZ WordPress Suite', 'koz-404-guard' ); ?></h1>
		<p><?php echo esc_html__( 'Independent WordPress tools shaped by real production use on the UA FREE charitable foundation website.', 'koz-404-guard' ); ?></p>
		<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:16px;max-width:1400px">
		<?php foreach ( self::status() as $component ) : ?><div style="background:#fff;border:1px solid #dcdcde;padding:16px"><h2 style="margin-top:0"><?php echo esc_html( $component['name'] ); ?></h2><p><strong><?php echo $component['active'] ? esc_html__( 'Active', 'koz-404-guard' ) : ( $component['installed'] ? esc_html__( 'Installed but inactive', 'koz-404-guard' ) : ( $component['legacy_active'] ? esc_html__( 'Legacy UA FREE version active', 'koz-404-guard' ) : esc_html__( 'Not installed', 'koz-404-guard' ) ) ); ?></strong><?php if ( '' !== $component['version'] ) : ?> <code><?php echo esc_html( $component['version'] ); ?></code><?php endif; ?></p><?php if ( $component['active'] && '' !== $component['settings_page'] ) : ?><p><a class="button" href="<?php echo esc_url( admin_url( 'admin.php?page=' . $component['settings_page'] ) ); ?>"><?php echo esc_html__( 'Open', 'koz-404-guard' ); ?></a></p><?php endif; ?></div><?php endforeach; ?>
		</div></div><?php
	}
}
