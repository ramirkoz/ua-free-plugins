...
final class UAFree_SEO_Scanner {
	...
	public static function scan( bool $deep = false ): array {
		...
		if ( $deep ) {
			$all_meta = array( 'uafree_seo_title', 'uafree_seo_description', 'uafree_seo_canonical' );
		...
		if ( $deep ) {
			$own_data['titles']       = (int) ( $meta_counts['uafree_seo_title'] ?? 0 );
			$own_data['descriptions'] = (int) ( $meta_counts['uafree_seo_description'] ?? 0 );
			$own_data['canonicals']   = (int) ( $meta_counts['uafree_seo_canonical'] ?? 0 );
		}
