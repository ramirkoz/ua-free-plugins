<?php
/**
 * Bundled runtime dictionaries for KOZ Suite administration screens.
 *
 * The current WordPress user locale is used in wp-admin. The site locale is
 * used on the frontend. Ukrainian, Chinese, Spanish, Arabic, Indonesian,
 * Portuguese, French, Japanese, German and Hindi are bundled. English is the
 * fallback for English and every unsupported locale.
 */
...
if ( ! class_exists( __NAMESPACE__ . '\\KOZSUITECC_Runtime_I18n', false ) ) {
	final class KOZSUITECC_Runtime_I18n {
		...
		public static function ngettext( string $translation, string $single, string $plural, int $number, string $domain ): string {
			return isset( self::$domains[ $domain ] )
				? self::translate( 1 === $number ? $single : $plural, $domain )
				: $translation;
		}
