...
final class KOZTDIAG_Suite_Registry {
	private const FALLBACK_PAGE = 'koztdiag-suite';
	...
	public static function suite_page(): string {
		...
		add_menu_page(
			__( 'KOZ WordPress Suite', 'koz-translate-diagnostics' ),
			__( 'KOZ Suite', 'koz-translate-diagnostics' ),
	...
	public static function render(): void {
		...
		?>
		<div class="wrap koztdiag-suite-overview"><h1><?php echo esc_html__( 'KOZ WordPress Suite', 'koz-translate-diagnostics' ); ?></h1>
		<p><?php echo esc_html__( 'Independent WordPress tools shaped by real production use on the UA FREE charitable foundation website.', 'koz-translate-diagnostics' ); ?></p>
		<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:16px;max-width:1400px">
		<?php foreach ( self::status() as $component ) : ?><div style="background:#fff;border:1px solid #dcdcde;padding:16px"><h2 style="margin-top:0"><?php echo esc_html( $component['name'] ); ?></h2><p><strong><?php echo $component['active'] ? esc_html__( 'Active', 'koz-translate-diagnostics' ) : ( $component['installed'] ? esc_html__( 'Installed but inactive', 'koz-translate-diagnostics' ) : ( $component['legacy_active'] ? esc_html__( 'Legacy UA FREE version active', 'koz-translate-diagnostics' ) : esc_html__( 'Not installed', 'koz-translate-diagnostics' ) ) ); ?></strong><?php if ( '' !== $component['version'] ) : ?> <code><?php echo esc_html( $component['version'] ); ?></code><?php endif; ?></p><?php if ( $component['active'] && '' !== $component['settings_page'] ) : ?><p><a class="button" href="<?php echo esc_url( admin_url( 'admin.php?page=' . $component['settings_page'] ) ); ?>"><?php echo esc_html__( 'Open', 'koz-translate-diagnostics' ); ?></a></p><?php endif; ?></div><?php endforeach; ?>
		</div></div><?php
	}
}
