...
final class KOZMIG_Environment_Scanner {
	private const CACHE_KEY = 'kozmig_environment_scan_v095';
	...
	public static function environment( bool $force = false ): array {
		...
		$report = array(
			...
			'cron' => $cron,
			'migration' => KOZMIG_Content_Migration_Scanner::report(),
			'suite' => KOZMIG_Suite_Registry::status(),
			'safety' => array(
	...
	public static function inspect_plugin( string $plugin_file ) {
		if ( ! function_exists( 'get_plugins' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}
		...
		if ( ! isset( $plugins[ $plugin_file ] ) ) {
			return new \WP_Error( 'plugin_not_found', __( 'The selected plugin is not installed.', 'koz-migration-cleanup' ) );
		}
		...
		$report = array(
			...
			'interpretation' => array(
				...
				'warning' => __( 'Matches are candidates, not proof of ownership. Cleanup requires a dedicated adapter and a verified snapshot.', 'koz-migration-cleanup' ),
			),
		);
