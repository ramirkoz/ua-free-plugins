(() => {
	...
	const config = window.UAFreeDonateStats || {};
	...
	function managerAllows(category) {
		try {
			if (
				window.UAFreeConsentManager &&
				typeof window.UAFreeConsentManager.allows === 'function'
			) {
				return Boolean(window.UAFreeConsentManager.allows(category));
			}

			if (
				window.UAFreeConsent &&
				typeof window.UAFreeConsent.allows === 'function'
			) {
				return Boolean(window.UAFreeConsent.allows(category));
			}

			if (typeof window.uafreeConsentAllows === 'function') {
				return Boolean(window.uafreeConsentAllows(category));
			}
		} catch (error) {
			return false;
		}
	...
	function pushExternalEvent(eventType, targetKey) {
		...
		window.dataLayer.push({
			event: String(config.dataLayerEvent || 'uafree_donation_event'),
			uafree_event_type: detail.event_type,
			uafree_target_key: detail.target_key,
			uafree_context_key: detail.context_key,
			uafree_language: detail.language,
			uafree_ad_account_mode: detail.ad_account_mode
		});
	}
	...
	function handleInteraction(target) {
		...
		if (copy) {
			// UA FREE Copy reports only a successful clipboard write.
	...
	function start() {
		...
		document.addEventListener(
			...
			event => {
				...
				if (String(detail.plugin || '') !== 'ua-free-copy') {
					return;
				}
