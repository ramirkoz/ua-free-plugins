...
final class Environment_Scanner {
	...
	private static function suggestions( array $installed ): array {
		...
		if ( isset( $installed['pagelayer/pagelayer.php'] ) ) {
			$suggestions[] = array(
				...
				'note'     => __( 'A service query key was detected. Review it before creating a 410 rule.', 'ua-free-404-guard' ),
			);
			$suggestions[] = array(
				'provider' => 'PageLayer',
				'type'     => 'query_pair',
				'value'    => 'post_type=pagelayer-template',
				'note'     => __( 'A service query pair was detected. Review it before creating a 410 rule.', 'ua-free-404-guard' ),
			);
		}
		return $suggestions;
	}
	...
	public static function internal_link_audit( int $limit = 100, bool $include_items = true ): array {
		...
		foreach ( $posts as $post_id_raw ) {
			...
			foreach ( self::extract_links( $content ) as $href ) {
				...
				if ( '' === $path || Guard::is_protected_path( $path ) || preg_match( '/\.[a-z0-9]{2,8}$/i', $path ) ) {
					continue;
				}
				$key = $post_id . '|' . $path;
				if ( isset( $seen[ $key ] ) ) {
				...
				$item = array(
					'path_fingerprint' => Guard::fingerprint( $path, 'internal-link' ),
				);
				if ( $include_items ) {
	...
	private static function local_candidate_path( string $href, string $home_host, int $home_port ): string {
		...
		return Guard::normalise_path( $parsed['path'] );
	}
}
