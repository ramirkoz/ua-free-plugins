(() => {
	...
	const config = window.UAFreeCopyConfig || {};
	const selectors = Array.isArray(config.selectors) ? config.selectors.filter((item) => typeof item === 'string' && item) : [];
	if (!selectors.length) {
		return;
	}

	const language = (document.documentElement.lang || 'en').toLowerCase().startsWith('uk') ? 'uk' : 'en';
	const messages = (config.messages && config.messages[language]) || (config.messages && config.messages.en) || {
		copied: 'Copied',
	...
	function closestTarget(start) {
		...
		while (node && node !== document.documentElement) {
			if (node.hasAttribute('data-uafree-copy-ignore')) {
				return null;
			}
			for (const selector of selectors) {
				if (safeMatches(node, selector)) {
					return node;
				}
			}
	...
	function valueFromTarget(element) {
		if (element.hasAttribute('data-copy-value')) {
			return normalizeValue(element.getAttribute('data-copy-value'));
		}
	...
	async function writeClipboard(value) {
		...
		textarea.remove();
		if (!copied) {
			throw new Error('Legacy clipboard API failed');
		}
	}

	function targetKey(element) {
		const explicit = element.getAttribute('data-copy-key') || element.getAttribute('data-uafree-copy') || '';
		if (/^[A-Za-z0-9_-]{1,80}$/.test(explicit)) {
			return explicit;
		}
		if (/^[A-Za-z][A-Za-z0-9_-]{0,79}$/.test(element.id || '')) {
			return element.id;
		}
		return 'copy';
	}
	...
	function showToast(message, isError) {
		...
		let toast = document.getElementById('uafree-copy-toast');
		if (!toast) {
			toast = document.createElement('div');
			toast.id = 'uafree-copy-toast';
			toast.className = `uafree-copy-toast-${config.toastPosition || 'bottom-center'}`;
	...
	function decorate(element) {
		if (!(element instanceof HTMLElement) || element.dataset.uafreeCopyDecorated === '1') {
			return;
		}
		element.dataset.uafreeCopyDecorated = '1';
		element.classList.add('uafree-copy-target');
		if (config.showIcon) {
			element.classList.add('uafree-copy-icon');
		}
		if (!element.hasAttribute('title')) {
			element.setAttribute('title', messages.hint);
		}
		if (config.decorateTargets && !isNativeInteractive(element)) {
			...
			if (!element.hasAttribute('aria-label')) {
				element.setAttribute('aria-label', messages.hint);
			}
		}
	}
	...
	function scan(root) {
		...
		for (const selector of selectors) {
			try {
				...
				// Server validation rejects unsupported selectors; the frontend still fails closed.
			}
		}
	}
	...
	async function activate(element, event) {
		...
		try {
			...
			element.classList.add('uafree-copy-success');
			showToast(messages.copied, false);
			dispatchSuccess(element);
			window.setTimeout(() => element.classList.remove('uafree-copy-success'), 700);
		} catch (error) {
